...
from panels.voice_chat_panel import VoiceRoomPanel
...
class Client(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.playing_stream = self.p.open(format=pyaudio.paInt16, channels=1, rate=20000, output=True,
                                          frames_per_buffer=1024)
        self.recording_stream = self.p.open(format=pyaudio.paInt16, channels=1, rate=20000, input=True,
                                            frames_per_buffer=1024)
        ...
        self.voice_room_win = VoiceRoomPanel(self, signal=self.connect_monitor.mysignal)
    ...
    def signal_handler(self, value: dict):
        ...
        if action == "SERVER_OK":
            ...
            self.active_page.rooms_ui.voice_chats.itemDoubleClicked.connect(
                self.test
            ...
            self.room_type = value['data'][2]
            if value['data'][2] == 'voice':
                self.change_active_window_to(self.voice_room_win)
                self.signal_handler({'action': 'SEND_VOICE', 'data': '' })
            else:
                self.change_active_window_to(self.text_room_win)
            ...
            if self.room_type == 'text':
                ...
                self.active_page.text_chat_ui.message_list.addItem(item)
            elif self.room_type == 'voice':
                print(value['data'][1])
                self.playing_stream.write(value['data'][1])


        elif action == "SEND_VOICE":
            voice = self.recording_stream.read(1024)
            self.send_message(action=action, data=[self.room, voice])
