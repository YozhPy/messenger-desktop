...
class Server:
    ...
    def message_handler(self, client_socket: socket.socket):
        while True:
            ...
            # Get all available rooms
                ...
                if payload['data'][2] == 'voice':
                    print(f"here123: {payload}")
                    client_socket.send(pickle.dumps(payload))
                    self.broadcast({'action': 'VOICE', 'data':[payload['data'][0], b''], 'auth_key': pickle_data['auth_key'] })
            else:
                payload = {
                    'action': 'ERROR',
    ...
    def exit_rooms(self, pickle_data: dict) -> dict:
        user = User.get_by_auth_key(pickle_data['auth_key'])
        Room.remove_user_from_room(pickle_data['data'], user['_id'])
    ...
    def try_to_connect_to_room(self, pickle_data: dict) -> dict:
        ...
        try:
            ...
            Room.add_user_to_room(room_name=room['name'], user_id=user['_id'])
            return {'action': 'CONNECTED_TO_ROOM_SUCCESSFULLY', 'data': (room['name'], room['messages'], room['type'])}
    ...
    def broadcast(self, pickle_data):
        while True:
            ...
            # while True:
                # print('packet')
            packet = self.connected_clients[pickle_data['auth_key']].recv(64000)
                # if not packet: break
                # message += packet
...
if __name__ == "__main__":
    myserver = Server('127.0.0.1', 5400)
